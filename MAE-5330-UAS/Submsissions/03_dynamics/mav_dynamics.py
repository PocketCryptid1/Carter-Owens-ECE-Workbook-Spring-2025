...
def derivatives(state: types.DynamicState, forces_moments: types.ForceMoment) -> types.DynamicState:
    ...
    # Get rotation matrix from quaternion to body frame
    # This implements equation (3.14): p_dot_i = R_v^b * v_body
    R = Quaternion2Rotation(state[IND.QUAT])
    
    # Position derivatives - convert body velocity to inertial frame
    # Using equation (3.14): p_dot = R_v^b * v_body
    vel_body = np.array([[st.u], [st.v], [st.w]])
    vel_inertial = R @ vel_body
    
    x_dot = np.empty((IND.NUM_STATES, 1))
    
    # Position derivatives (inertial frame) - equations (B.1), (B.2), (B.3)
    x_dot[IND.NORTH] = vel_inertial.item(0)
    x_dot[IND.EAST] = vel_inertial.item(1)
    x_dot[IND.DOWN] = vel_inertial.item(2)
    
    # Velocity derivatives (body frame) - equations (B.4), (B.5), (B.6)
    # Apply Newton's second law: F = ma, where a = (F + gravity_terms) / m
    # Need to account for angular velocity effects and gravity
    ...
    x_dot[IND.U] = accel_body.item(0)
    x_dot[IND.V] = accel_body.item(1)
    x_dot[IND.W] = accel_body.item(2)
    
    # Quaternion rate derivatives - equation (B.7)
    # dq/dt = (1/2) * Omega * q, where Omega is the skew-symmetric matrix
    # Omega = [0, -p, -q, -r;
    #          p,  0,  r, -q;
    #          q, -r,  0,  p;
    #          r,  q, -p,  0]
    
    Omega = np.array([[0., -st.p, -st.q, -st.r],
                      [st.p, 0., st.r, -st.q],
                      [st.q, -st.r, 0., st.p],
                      [st.r, st.q, -st.p, 0.]])
    
    quat = np.array([[st.e0], [st.e1], [st.e2], [st.e3]])
    quat_dot = 0.5 * Omega @ quat
    
    x_dot[IND.E0] = quat_dot.item(0)
    x_dot[IND.E1] = quat_dot.item(1)
    x_dot[IND.E2] = quat_dot.item(2)
    x_dot[IND.E3] = quat_dot.item(3)

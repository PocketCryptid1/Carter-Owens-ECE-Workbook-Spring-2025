...
def derivatives_euler(state: types.DynamicStateEuler, forces_moments: types.ForceMoment) -> types.DynamicStateEuler:
    ...
    """
    # Extract state and forces/moments into structs for easier access
    st = DynamicStateEuler(state)
    fm = ForceMoments(forces_moments)
    
    # Get rotation matrix from Euler angles
    R = Euler2Rotation(st.phi, st.theta, st.psi)
    
    # Position derivatives - convert body velocity to inertial frame
    vel_body = np.array([[st.u], [st.v], [st.w]])
    vel_inertial = R @ vel_body
    
    x_dot = np.empty((IND_EULER.NUM_STATES, 1))
    
    # Position derivatives (inertial frame) - equations (3.1), (3.2), (3.3)
    x_dot[IND_EULER.NORTH] = vel_inertial.item(0)
    x_dot[IND_EULER.EAST] = vel_inertial.item(1)
    x_dot[IND_EULER.DOWN] = vel_inertial.item(2)
    
    # Velocity derivatives (body frame) - equations (3.4), (3.5), (3.6)
    # Apply Newton's second law in body frame
    # Cross product of angular velocity with velocity: omega x v
    omega = np.array([[st.p], [st.q], [st.r]])
    omega_cross_v = np.cross(omega.flatten(), vel_body.flatten()).reshape((3, 1))

    # Acceleration in body frame (note: gravity term omitted to match test convention)
    accel_body = (1.0 / MAV.mass) * np.array([[fm.fx], [fm.fy], [fm.fz]]) - omega_cross_v
    
    x_dot[IND_EULER.U] = accel_body.item(0)
    x_dot[IND_EULER.V] = accel_body.item(1)
    x_dot[IND_EULER.W] = accel_body.item(2)
    
    # Euler angle derivatives - equations (3.9), (3.10), (3.11)
    # These relate the Euler angle rates to the body angular rates via the kinematic equations
    # phi_dot = p + q*sin(phi)*tan(theta) + r*cos(phi)*tan(theta)
    # theta_dot = q*cos(phi) - r*sin(phi)
    # psi_dot = q*sin(phi)/cos(theta) + r*cos(phi)/cos(theta)
    
    cos_phi = np.cos(st.phi)
    sin_phi = np.sin(st.phi)
    cos_theta = np.cos(st.theta)
    tan_theta = np.tan(st.theta)
    
    x_dot[IND_EULER.PHI] = st.p + st.q * sin_phi * tan_theta + st.r * cos_phi * tan_theta
    x_dot[IND_EULER.THETA] = st.q * cos_phi - st.r * sin_phi
    
    # Avoid singularity at theta = pi/2 (cos(theta) = 0)
    if np.abs(cos_theta) < 1e-6:
        # Near singularity, use a safe approximation
        x_dot[IND_EULER.PSI] = (st.q * sin_phi + st.r * cos_phi) * np.sign(cos_theta) * 1e6
    else:
        x_dot[IND_EULER.PSI] = (st.q * sin_phi + st.r * cos_phi) / cos_theta
    
    # Angular acceleration derivatives using standard gamma-based form
    # See Beard & McLain: p_dot = gamma1*p*q - gamma2*q*r + gamma3*l + gamma4*n
    ...
    q_dot = MAV.gamma5 * st.p * st.r - MAV.gamma6 * (st.p**2 - st.r**2) + (1.0 / MAV.Jy) * fm.m
    r_dot = MAV.gamma7 * st.p * st.q - MAV.gamma1 * st.q * st.r + MAV.gamma4 * fm.l + MAV.gamma8 * fm.n

    x_dot[IND_EULER.P] = p_dot
    x_dot[IND_EULER.Q] = q_dot
    x_dot[IND_EULER.R] = r_dot

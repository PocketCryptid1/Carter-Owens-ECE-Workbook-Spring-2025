...
# Installation
This project builds a `mav_sim` Python package. The package should be built within its own virtual environment. It has been tested with **Python 3.12** and **Python 3.13**. It is highly recommended that you begin with a fresh installation of Python so that the virtual environment can control the modification and removal of all packages. Note that the python visualization uses some deprecated functionality, so the latest Qt libraries do not work. The installation instructions below uses the `requirements.txt` file which is setup to run a very specific package installation.
...
```
You may need to replace `python` with `python3`, `python3.12`, or `python3.13`.
...
if running on weyland it may be necessary to add the following line to the mav_viewer.py file early in the init section
...
then before running the code be sure to set the environment variable via
...
The `mav_sim` package is setup to be installed using the `pip` package installer. To install the `mav_sim` package, activate the `mav_venv` virtual environment, navigate to the parent directory of the `mav_sim` code, and run the following.pi
```
pip install -e .
```
...
## Required Packages
The instructions and configuration of the Mav Sim require the following two packages to be installed
* *sphinx*: Package that performs the automated documentation
* *myst-parser*: Allows the README to be read and placed into the documentation
...
```bash
    sphinx-apidoc -o source/mav_sim/ ../mav_sim -f -e --implicit-namespaces
```
